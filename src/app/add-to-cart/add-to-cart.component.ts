...
export class AddToCartComponent implements OnInit {
  ...
  //end 21/08/2020
   ...
   if( this.estimate_self==true ){
        this.otpForm.get("delivery_pincode").setValue('');
  //.console.log(1);
           this.estimate_time_delivery();
      }
  //end for estimate time delivery 24/08/2020
             if(this.is_estimate==true && this.estimate_self==false){
              for (let i = 0; i < this.products.length; i++) {
                 this.otpForm.get("length").setValue(this.products[i].product.length);
               this.otpForm.get("width").setValue(this.products[i].product.breadth);
               this.otpForm.get("height").setValue(this.products[i].product.height);
                this.otpForm.get("invoicevalue").setValue(this.products[i].product_rate[0].rate_qty);
               console.log(this.products[i].product_rate[0].rate_qty);
            // invoicevalue
        ...
        this.adminService.fetchCart(postData).subscribe(data => {
          ...
          this.adminService.buy=false;
            ...
            // }
              ...
              if(this.is_estimate==true && this.estimate_self==false){
                   this.otpForm.get("length").setValue(this.c[check].product.length);
               this.otpForm.get("width").setValue(this.c[check].product.breadth);
               this.otpForm.get("height").setValue(this.c[check].product.height);
                this.otpForm.get("invoicevalue").setValue(this.c[check].rate+this.c[check].shipping_amt);
               
           console.log(this.c[check].rate+this.c[check].shipping_amt);
  ...
  // for delete cart and move to wishlist in login 
    deleteFromCartw(id) {
  ...
  // product move in wishlist
    addToWishlistDl(product) {
      console.log(product);
      var postData = {
        user_num: "",
        access_token: "",
      ...
      postData.user_num = sessionStorage.getItem("user_num");
      postData.access_token = sessionStorage.getItem("access_token");
      postData.product_no = product;
      postData.comp_num = sessionStorage.getItem("comp_num_new");
      
  
      if (postData.user_num == "" || postData.access_token == "") {
        this.snackbar.open("Please Login To Move Product in Wishlist", "", {
          duration: 1000
        });
        ...
        if (product.is_wishlist == "True") {
          ...
          this.adminService.addWishlist(postData).subscribe(data => {
            ...
            this.snackbar.open("product moved successfully ", "", {
              duration: 1000
            });
            this.ngOnInit();
          });
  ...
  // }
   ...
   updateCart3(cart_id,cart_inventory_id,rate_type_actual,qty,qty_stock) {
      ...
      if (qty < 1) {
        this.deleteFromCart(cart_inventory_id);
        // this.snackbar.open("Choose Valid quantity.", "", {
        //   duration: 3000
        // });
        // this.getCart();
           ...
           if(v == '2' || v == '3'){
             ...
             this.rentShow = false;
                ...
                if(data['data'][k].s_no==6 || data['data'][k].s_no=='6'){
                  if(data['data'][k].value!=3 || data['data'][k].value!='3'){
      ...
      this.adminService.fetchAddress(postData).subscribe(data => {
        if(data["result"].length>0){
          // this.addresses_pincode=data["result"][0].pin_code;
                this.otpForm.get("delivery_pincode").setValue(data["result"][0].pin_code);
  ...
  console.log(this.estimate_count);
   // this.load++;
        ...
        if(this.userC==true){
            if(this.c.length>0){
              // for (let i = 0; i < this.c.length; i++) {
               ...
               //    this.otpForm.get("length").setValue(this.c[0].product.length);
               // this.otpForm.get("width").setValue(this.c[0].product.breadth);
               // this.otpForm.get("height").setValue(this.c[0].product.height);
               //  this.otpForm.get("invoicevalue").setValue(this.c[0].rate+this.c[0].shipping_amt);
               
               // this.otpForm.get("weight").setValue(this.c[0].product.final_weight);
            ...
            if(this.products.length>0){
              // for (let i = 0; i < this.products.length; i++) {
               ...
               //   this.otpForm.get("length").setValue(this.products[0].product.length);
               // this.otpForm.get("width").setValue(this.products[0].product.breadth);
               // this.otpForm.get("height").setValue(this.products[0].product.height);
               //  this.otpForm.get("invoicevalue").setValue(this.products[0].product_rate[0].rate_qty);
           
               // this.otpForm.get("weight").setValue(this.products[0].product.final_weight);
